...
# Disable Google Cloud metadata server detection (force credentials file usage)
os.environ['GCE_METADATA_HOST'] = 'DISABLED'

# Google Cloud clients - explicitly use credentials file
...
if twilio_creds:
    if 'api_key' in twilio_creds and 'api_key_secret' in twilio_creds:
        ...
        # Note: account_sid must be passed as keyword argument for API key auth
...
def synthesize_speech_url(text, language_code, conference_name):
    ...
    try:
        ...
        # Voice configuration based on language - use natural sounding voices
        if language_code == 'hi':
            voice = texttospeech.VoiceSelectionParams(
                language_code="hi-IN",
                name="hi-IN-Wavenet-A",  # Wavenet for more natural sound
                ssml_gender=texttospeech.SsmlVoiceGender.FEMALE
            )
        else:
            voice = texttospeech.VoiceSelectionParams(
                language_code="en-US",
                name="en-US-Wavenet-C",  # Wavenet for more natural sound
                ssml_gender=texttospeech.SsmlVoiceGender.FEMALE
            )
        
        # Audio configuration - optimized for phone calls
        audio_config = texttospeech.AudioConfig(
            audio_encoding=texttospeech.AudioEncoding.MP3,
            speaking_rate=1.1,  # Slightly faster for more natural conversation flow
            pitch=0.0,
            ...
            if event == 'connected':
                ...
                if len(audio_buffer) >= 6000:
                    try:
                        ...
                        # Assign fixed languages based on participant role for consistency
                        # Caller speaks English, Receiver speaks Hindi
                        if participant_role == "caller":
                            source_lang = "en"
                            target_lang = "hi"
                            speech_lang = "en-US"
                            target_role = "receiver"
                            speech_model = "phone_call"  # phone_call model available for English
                        else:  # receiver
                            source_lang = "hi"
                            target_lang = "en"
                            speech_lang = "hi-IN"
